...
int main()
{
	...
	for (double kappa = min_kappa; kappa <= max_kappa; kappa += step_kappa) {
		...
		for (int k = 0; k < sol.size(); k++) {
			left_part_of_kernel.push_back((pow((sol[k][1] - lambda_1 / points_xi[k] * sol[k][0]) / (lambda_1 + 2.0 * mu), 2) + pow(sol[k][0] / points_xi[k], 2)) * tau);
		}
		matrix_of_kernel_1.push_back(left_part_of_kernel);

		for (int k = 0; k < sol.size(); k++) {
			tmp_right_part_of_kernel_2.push_back(tau * (pow(sol[k][0], 2) / (inner_radius * points_xi[k])));
		}
	...
	const VoyevodinMethod voyevodin_method(matrix_left_part_of_kernel.getElements(), right_part, step_kappa, Dirichle, Dirichle);
