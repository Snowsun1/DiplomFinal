...
int main()
{
	...
	for (double kappa = min_kappa; kappa <= max_kappa; kappa += step_kappa) {
		...
		std::vector<std::complex<double>> tmp_sol_1;
		std::vector<std::complex<double>> tmp_sol_2;

		for (int k = 0; k < 20; k++) {
			tmp_sol_1.push_back((pow((sol[k][1] - lambda_1 / points_xi[k] * sol[k][0]) / (lambda_1 + 2.0 * mu), 2) + pow(sol[k][0] / points_xi[k], 2)) * tau);
		}
		matrix_of_kernel_1.push_back(tmp_sol_1);

		for (int k = 0; k < 20; k++) {
			tmp_sol_2.push_back(tau * (pow(sol[k][0], 2) / (inner_radius * points_xi[k])));
		}
		matrix_of_kernel_2.push_back(tmp_sol_2);
