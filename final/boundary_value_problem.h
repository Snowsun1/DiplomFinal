...
class boundary_value_problem
{
	...
	double inner_radius; // внутренний радиус цилиндра
...
public:
	boundary_value_problem(
		...
		std::map<size_t, T> right_conditions, double inner_radius, double epsilon = 0.1e-6);
...
boundary_value_problem<T>::cauchy_problem_solutions() const
{
	...
	for (size_t i = 0; i < size; i++)
	{
		...
		auto solution = Parameters::kind == THIRD ?
			ode_solver.solve(Parameters::points, initials, inner_radius).back() :
			ode_solver.solve(inner_radius, 1, initials);
...
boundary_value_problem<T>::boundary_value_problem(
	...
	double inner_radius, double epsilon) :
...
std::vector<T> boundary_value_problem<T>::solve() const
{
	...
	for (size_t i = 0; i < solutions.size(); i++)
	{
		...
		{
			result[ii] += initial_conditions[i] * solutions[i][ii];
		}
	}
	return result;
}
	...
	const std::vector<double>& points) const
{
	...
	OdeSolver<T> ode_solver(_equations, this->_epsilon, RKF_78);
	return ode_solver.solve(points, initials, inner_radius);
}